...
const Dropwdown = ({ header, selected, list, selectedOnPress }) => {
  ...
  useEffect(() => {
    setFilteredList(list.filter((item) => item.category.includes(searchInput)));
  }, [searchInput]);
  return (
    <View style={styles.container}>
      ...
      {dropdownIsOpen ? <ScrollView style={styles.dropdown_container}>{listMap()}</ScrollView> : ''}
    </View>
  );
};
